...
func TestUndergroundSystem(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: []interface{}{nil, nil, nil, nil, nil, nil, nil, 14.00000, 11.00000, nil, 11.00000, nil, 12.00000},
		},
		{
			...
			expected: []interface{}{nil, nil, nil, 5.00000, nil, nil, 5.50000, nil, nil, 6.66667},
		},
		{
			...
			expected: []interface{}{nil, nil, nil, 10.00000, nil, nil, 10.00000, nil, 45.00000, nil, 10.00000, 45.00000, nil},
		},
	}
	...
	for _, test := range tests {
		...
		for i := 0; i < len(test.actions); i++ {
			...
			case "getAverageTime":
				...
				if at != test.expected[i].(float64) {
					t.Errorf("Test failed. average time is expected to %f but response %f", test.expected[i], at)
				}
			}
		}
	}
